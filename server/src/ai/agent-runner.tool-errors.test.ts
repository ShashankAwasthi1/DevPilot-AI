...
const boomTool = {
  ...
  handler: async () => {
    throw new Error("simulated internal failure - this text must never reach the model or the browser");
  },
};

test("runAgentTurn: unknown tool, malformed arguments, and a handler failure are all handled without crashing the turn or leaking details", async (t) => {
  const { provider, calls } = makeScriptedProvider((callIndex) => {
    if (callIndex === 0) {
      return [
        { type: "tool_use", id: "call-unknown", name: "doesNotExist", input: {} },
        { type: "tool_use", id: "call-malformed", name: "echoTool", input: { value: 123 } },
  ...
  assert.ok(done, "the run must recover and still reach a final done event, not crash");
